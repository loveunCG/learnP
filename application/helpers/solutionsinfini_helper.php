...
class sendsms
{
	...
	function __construct($method = 'sms')
	{
		...
		$query = 'SELECT * FROM '.$this->_ci->db->dbprefix('system_settings_types').' sst INNER JOIN '.$this->_ci->db->dbprefix('system_settings_fields').' ssf ON sst.type_id = ssf.type_id WHERE sst.type_title = "Solutionsinfini" ORDER BY field_name';

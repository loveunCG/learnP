...
class nexmo
{
    ...
    public function __construct()
    {
		...
		*/
		...
		$query = 'SELECT * FROM '.$this->_ci->db->dbprefix('system_settings_types').' sst INNER JOIN '.$this->_ci->db->dbprefix('system_settings_fields').' ssf ON sst.type_id = ssf.type_id WHERE sst.type_title = "Nexmo" ORDER BY field_name';
		...
		if(count($gateway_details) > 0) {
			foreach($gateway_details as $selectedgateway) {
				...
				{
					case 'API_KEY':
						$this->_api_key = $selectedgateway->field_output_value;
					break;
					case 'API_SECRET':
						$this->_api_secret = $selectedgateway->field_output_value;
					break;
				}						
			}
		}

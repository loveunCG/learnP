	...
	*/
		...
		else
		{
			...
			else
			{
				...
				$email_template = $this->db->query('SELECT * FROM '.$this->db->dbprefix('email_templates').' WHERE email_template_id = 1 AND template_status = "Active" ORDER BY email_template_id ASC LIMIT 1')->result();
				if(!empty($email_template))
				{
					...
					$content 	= str_replace("__FIRST_NAME__", $additional_data['first_name'], $content);
					$content 	= str_replace("__LAST_NAME__", $additional_data['last_name'], $content);
					...
					if($fb)
					{
					$content 	= str_replace("__ACTIVATELINK__", '',$content);
					}
					else
					{
					$content 	= str_replace("__ACTIVATELINK__", SITEURL2.'/auth/activate/'.$id.'/'.$activation_code,$content);						
					}
